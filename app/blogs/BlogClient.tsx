...
export default function BlogClient({ posts }: { posts: any[] }) {
  ...
  // Helper to produce a stable key for posts
  const getPostKey = (post: any, index: number) =>
    post?.id ?? post?.slug ?? `${(post?.title ?? "post")}-${index}`;
      ...
      <section className="bg-black m-none">
        <div className="container">
          ...
          >
            <h1 className="text-4xl md:text-6xl font-bold">
              Saavik Solutions{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#6A43E7] via-[#7F38E8] to-[#E879F9] sm:inline">
                Blogs
              </span>
            </h1>
            <p className="text-muted-foreground text-lg">
              Stay updated with the latest insights on AI technology, digital
              marketing trends, and industry best practices
            </p>
      ...
      <div className="container mt-12 flex flex-col lg:flex-row">
        ...
        <div className="flex-1 lg:pl-6">
          ...
          <section className="w-full pt-12">
            <header className="flex items-center justify-between mb-4">
              <h2 className="text-gray-500 text-xl font-bold">
                <span className="text-purple-700">{activeCategory}</span>{" "}
                Articles
              </h2>
              <p className="text-xl text-gray-700">
